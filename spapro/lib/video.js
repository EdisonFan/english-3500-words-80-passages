...
function pipeMp4(mp4Url, req, res, onHeadersSent) {
  return new Promise((resolve, reject) => {
    ...
    httpsRequestStream(mp4Url, headers, 30000).then(({ statusCode, headers: upHeaders, stream, session }) => {
      const outHeaders = {};
      ...
      stream.on('end', () => { if (session) session.close(); resolve(); });
      stream.on('error', (e) => { if (session) session.close(); reject(e); });
    }).catch(reject);
  });
}
